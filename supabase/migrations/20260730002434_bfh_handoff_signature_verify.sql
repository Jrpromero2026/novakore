-- Final handoff exchange: verify the BFH HMAC signature + timing INSIDE the
-- RPC so the per-org secret never leaves the database. Canonical signing
-- input (both sides build it identically):
--   v1|orgSlug|externalUserId|email|accessLevel|audiencesCsv|issuedAt|expiresAt|nonce
-- where audiencesCsv is the audiences sorted ascending and comma-joined.
drop function if exists public.bfh_exchange_handoff(text,text,text,text,text,text[],text,bigint);
